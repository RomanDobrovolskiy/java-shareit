...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto addItem(ItemDto itemDto, Long userId) {
        userService.getUserById(userId); // проверка существует ли владелец вещи
    ...
    public ItemDto updateItem(ItemDto itemDto, Long itemId, Long userId) {
        ...
        if (item.getOwnerId().equals(userId)) {
            ...
            throw new UserIsNotOwnerException(String.format("Пользователь с id %d не является владельцем предмета", userId));
        }
    }
    ...
    public Comment addComment(Comment comment, Long userId, Long itemId) {
        ...
        if (bookings.isEmpty()) {
            throw new UserHasNotBookedItem("Нужно завершить хотя бы одно бронирование, чтобы оставлять комментарии");
        }
